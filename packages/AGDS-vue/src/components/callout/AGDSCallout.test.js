...
import AusGovCallout from './AusGovCallout.vue';
const AXE_OPTS = {
    rules: { 'color-contrast': { enabled: false } },
};
// ─── Helpers ─────────────────────────────────────────────────────────────────
function renderCallout(props = {}, slot = '<p>Callout content</p>') {
    return render(AusGovCallout, {
        props,
        slots: { default: slot },
    });
}
// ─── Rendering ───────────────────────────────────────────────────────────────
describe('AusGovCallout — rendering', () => {
    it('renders a <div> by default', () => {
        const { container } = renderCallout();
        expect(container.querySelector('div.ausgov-callout')).toBeTruthy();
    });
    ...
    it('does not render a title element when title is omitted', () => {
        const { container } = renderCallout();
        expect(container.querySelector('.ausgov-callout__title')).toBeNull();
    });
});
// ─── Props: as ───────────────────────────────────────────────────────────────
describe('AusGovCallout — as prop', () => {
    it('renders as <aside> when as="aside"', () => {
        const { container } = renderCallout({ as: 'aside' });
        expect(container.querySelector('aside.ausgov-callout')).toBeTruthy();
    });
    it('renders as <section> when as="section"', () => {
        const { container } = renderCallout({ as: 'section' });
        expect(container.querySelector('section.ausgov-callout')).toBeTruthy();
    });
});
// ─── Props: tone ─────────────────────────────────────────────────────────────
describe('AusGovCallout — tone prop', () => {
    it('defaults to neutral tone', () => {
        const { container } = renderCallout();
        expect(container.querySelector('.ausgov-callout--neutral')).toBeTruthy();
    });
    it.each(['neutral', 'info'])('applies ausgov-callout--%s class for tone="%s"', (tone) => {
        const { container } = renderCallout({ tone });
        expect(container.querySelector(`.ausgov-callout--${tone}`)).toBeTruthy();
    });
    it('shows the default info icon when tone=info and no icon slot is provided', () => {
        const { container } = renderCallout({ tone: 'info' });
        expect(container.querySelector('.ausgov-callout__icon')).toBeTruthy();
    });
    it('does not render an icon for neutral tone by default', () => {
        const { container } = renderCallout({ tone: 'neutral' });
        expect(container.querySelector('.ausgov-callout__icon')).toBeNull();
    });
});
// ─── Props: variant ───────────────────────────────────────────────────────────
describe('AusGovCallout — variant prop', () => {
    it('defaults to regular variant', () => {
        const { container } = renderCallout();
        expect(container.querySelector('.ausgov-callout--regular')).toBeTruthy();
    });
    it.each(['compact', 'regular', 'feature'])('applies ausgov-callout--%s class for variant="%s"', (variant) => {
        const { container } = renderCallout({ variant });
        expect(container.querySelector(`.ausgov-callout--${variant}`)).toBeTruthy();
    });
});
// ─── Props: onBodyAlt ─────────────────────────────────────────────────────────
describe('AusGovCallout — onBodyAlt prop', () => {
    it('applies body-alt class when onBodyAlt=true and tone=neutral', () => {
        const { container } = renderCallout({ tone: 'neutral', onBodyAlt: true });
        expect(container.querySelector('.ausgov-callout--body-alt')).toBeTruthy();
    });
    it('does not apply body-alt class by default', () => {
        const { container } = renderCallout();
        expect(container.querySelector('.ausgov-callout--body-alt')).toBeNull();
    });
...
// ─── Slots ────────────────────────────────────────────────────────────────────
describe('AusGovCallout — icon slot', () => {
    it('renders a custom icon slot', () => {
        const { container } = render(AusGovCallout, {
            props: { tone: 'neutral' },
            slots: {
                default: '<p>Content</p>',
...
describe('AusGovCallout — info icon a11y', () => {
...
describe('AusGovCallout — axe accessibility', () => {
