...
test.describe('WAI-ARIA — textarea form control (real-browser, colour-contrast enabled)', () => {
  // WAI-ARIA: every textarea must have a programmatically associated label
  // via <label for> / id pairing so assistive technology can announce the field name.
  test('default — label is associated with textarea via for/id pairing, axe-clean', async ({ mount, page }) => {
    await mount(AGDSTextarea, {
      props: { label: 'Description', id: 'test-textarea' },
    })

    const results = await runAxe(page)
    expect(results.violations).toEqual([])

    // Verify <label for> matches the textarea id.
  ...
  // WAI-ARIA: disabled textareas must expose the disabled state via the native
  ...
  test('disabled — native disabled attribute present, axe-clean', async ({ mount, page }) => {
    await mount(AGDSTextarea, {
      props: { label: 'Notes', disabled: true, id: 'test-textarea-disabled' },
    })
  ...
  test('required — aria-required attribute present, axe-clean', async ({ mount, page }) => {
    await mount(AGDSTextarea, {
      props: { label: 'Message', required: true, id: 'test-textarea-required' },
    })

    const results = await runAxe(page)
    expect(results.violations).toEqual([])

    await expect(page.locator('textarea')).toHaveAttribute('aria-required', 'true')
  })
})
