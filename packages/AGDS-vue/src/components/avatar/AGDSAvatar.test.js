...
import AusGovAvatar from './AusGovAvatar.vue';
const AXE_OPTS = {
    rules: { 'color-contrast': { enabled: false } },
};
// ─── Helpers ─────────────────────────────────────────────────────────────────
function renderAvatar(props = {}, attrs = {}) {
    return render(AusGovAvatar, { props, attrs });
}
// ─── Initials derivation ──────────────────────────────────────────────────────
describe('AusGovAvatar — initials', () => {
    ...
    it('shows "?" for an empty name', () => {
        ...
        expect(container.querySelector('.ausgov-avatar').textContent).toBe('?');
    });
...
// ─── Rendering ───────────────────────────────────────────────────────────────
describe('AusGovAvatar — rendering', () => {
    it('renders a <span> element', () => {
        const { container } = renderAvatar({ name: 'Jane Doe' });
        expect(container.querySelector('span.ausgov-avatar')).toBeTruthy();
    });
    ...
    it('uses a custom aria-label when provided', () => {
        ...
        expect(container.querySelector('.ausgov-avatar').getAttribute('aria-label')).toBe('Avatar for Jane Doe');
    });
});
// ─── Props: tone ─────────────────────────────────────────────────────────────
describe('AusGovAvatar — tone prop', () => {
    it('defaults to neutral tone', () => {
        const { container } = renderAvatar({ name: 'Jane Doe' });
        expect(container.querySelector('.ausgov-avatar--neutral')).toBeTruthy();
    });
    it.each(['neutral', 'action'])('applies ausgov-avatar--%s class for tone="%s"', (tone) => {
        const { container } = renderAvatar({ name: 'Jane Doe', tone });
        expect(container.querySelector(`.ausgov-avatar--${tone}`)).toBeTruthy();
    });
});
// ─── Props: size ─────────────────────────────────────────────────────────────
describe('AusGovAvatar — size prop', () => {
    it('defaults to md size', () => {
        const { container } = renderAvatar({ name: 'Jane Doe' });
        expect(container.querySelector('.ausgov-avatar--md')).toBeTruthy();
    });
    it.each(['sm', 'md', 'lg', 'xl', 'xxl', 'xxxl'])('applies ausgov-avatar--%s class for size="%s"', (size) => {
        const { container } = renderAvatar({ name: 'Jane Doe', size });
        expect(container.querySelector(`.ausgov-avatar--${size}`)).toBeTruthy();
    });
});
// ─── aria-hidden ─────────────────────────────────────────────────────────────
describe('AusGovAvatar — aria-hidden', () => {
    it('sets aria-hidden="true" and removes role when aria-hidden="true"', () => {
        ...
        const el = container.querySelector('.ausgov-avatar');
...
describe('AusGovAvatar — axe accessibility', () => {
    ...
    it.each(['sm', 'md', 'lg', 'xl', 'xxl', 'xxxl'])('has no violations for size=%s', async (size) => {
        const { container } = renderAvatar({ name: 'Jane Doe', size });
        await runAxe(container, AXE_OPTS);
    });
