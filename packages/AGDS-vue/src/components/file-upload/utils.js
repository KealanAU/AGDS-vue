...
export const TOO_MANY_FILES_ERROR = {
...
export function formatFileExtension(ext) {
    return ext.replace(/^\./, '').toUpperCase();
}
export function getAcceptedFilesSummary(acceptProp) {
    if (!acceptProp?.length)
        return undefined;
    const allFileTypes = [];
...
export function getErrorSummary(errors, formattedMaxFileSize) {
    if (!errors?.length)
        return '';
    const codes = [...new Set(errors.map((e) => e.code))];
    if (codes.includes(ERROR_CODE_FILE_INVALID_TYPE))
        return 'Invalid file type';
    if (codes.includes(ERROR_CODE_FILE_TOO_LARGE))
...
export function convertFileToTooManyFilesRejection(file) {
 ...
 */
export function validateFile(file, acceptMap, maxSizeBytes) {
    const errors = [];
    if (acceptMap) {
