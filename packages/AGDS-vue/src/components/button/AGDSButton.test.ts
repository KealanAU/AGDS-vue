...
describe('AGDSButton — variant prop', () => {
  ...
  it.each(['primary', 'secondary', 'tertiary'] as const)(
    ...
    (variant) => {
      ...
      expect(getByRole('button').className).toContain(`agds-button--${variant}`)
    },
  )
})
...
describe('AGDSButton — size prop', () => {
  ...
  it.each(['sm', 'md', 'lg'] as const)(
    ...
    (size) => {
      ...
      expect(getByRole('button').className).toContain(`agds-button--${size}`)
    },
  )
})
...
describe('AGDSButton — disabled prop', () => {
  ...
  it('applies agds-button--disabled class when disabled', () => {
    const { getByRole } = renderButton({ disabled: true })
    expect(getByRole('button').className).toContain('agds-button--disabled')
  })
})
...
describe('AGDSButton — loading prop', () => {
  it('is not loading by default', () => {
    ...
    // No spinner in DOM
    expect(queryByRole('button')!.querySelector('.agds-button__spinner')).toBeNull()
  })

  it('renders spinner when loading=true', () => {
    const { getByRole } = renderButton({ loading: true })
    const spinner = getByRole('button').querySelector('.agds-button__spinner')
  ...
  it('applies agds-button--loading class when loading', () => {
    const { getByRole } = renderButton({ loading: true })
    expect(getByRole('button').className).toContain('agds-button--loading')
  })
})
